...
namespace Games
{
    class FastGame : BasicGame
    {
        TimeSpan interval;
        private DateTime timeForChoice;
        private char[,] gridMatrix = new char[3, 3];
        ...
        public override void GameLogic(BasicAccount Player1, BasicAccount Player2, GameStatsService Stats, AccountService Accounts)
        {
            ...
            interval = DateTime.Now - timeForChoice;
            if (interval.Seconds >= 3)
            {
                RandomChoice:
                int randY = new Random().Next(3);
                int randX = new Random().Next(3);
                if (gridMatrix[randY,randX] != ' ')
                    goto RandomChoice;
                cX = randX;
                cY = randY;
                gridMatrix[randY,randX] = playerTurn;
                timeForChoice = DateTime.Now;
                choiceMade = true;
            }
        ...
        public override void PrintGame(BasicAccount Player1, BasicAccount Player2)
        {
            ...
            if (gameLife)
            {
                ...
                Console.WriteLine($"Time: {interval.Seconds}");
            }
            else
            {
                if (drawResult)
